...
void RooLagrangianMorphOptimizer::setupMorphFunc(){
  if(this->morphFunc){
    ...
    this->config.setObservableName("xsection");
    this->config.setVertices(this->vertices);
    this->config.setFolders(this->temporaries_list);
    // set Interference to-do
    if(this->_nonInterfering.size()!=0){
      this->morphFunc = new RooLagrangianMorphFunc("morphing","morphing",this->config);
    }else{
      this->morphFunc = new RooLagrangianMorphFunc("morphing","morphing",this->config);
    }
  }
...
// to-check      if(pars[ipar]<var->getMin()) pars_limit[ipar] = var->getMin();
// to-check      if(pars[ipar]>var->getMax()) pars_limit[ipar] = var->getMax();
    }
  }
  return pars_limit;
}
...
{
  // constructor with interference setting -- TODO
  //this->_nonInterfering = nonInterfering;
  this->config.setObservableName(inputobservable.Data());
  this->config.setVertices(this->vertices);
  this->config.setFolders(this->initInputs(xsInputsArg)); 
  this->xsHelper = new RooLagrangianMorphFunc("xsHelper","xsHelper",this->config);
...
void RooLagrangianMorphOptimizer::setup(const ParamCardSet& startvalues){
  const RooArgSet* parameterlist = this->xsHelper->getParameterSet();
  ...
  for(size_t i=0; i<this->fnSamples; ++i){
    ...
    if(this->fXSContainerType == TH1::Class()){
      ...
      // TPair version
      TPair* p = RooLagrangianMorphFunc::makeCrosssectionContainer(1,0);
      f->Add(p);
    }

    // loop over the parameters
    this->xsHelper->setParameters(this->xsInputs[i].c_str());
    const RooArgSet* parset = this->xsHelper->getParameterSet();
...
void RooLagrangianMorphOptimizer::cloneFileContents(const TString& filename, bool addbenchmarks){
  ...
  if (!f || f->IsZombie()) {
//to-do    ERROR("Cannot open file '" << filename << "!");
  }
  TKey *key;
  ...
  while ((key = (TKey*)nextkey())) {
    ...
    if(addbenchmarks){
      ...
      if(!hist){
//to-do	ERROR("unable to read hist '"<<benchmarkobservable<<"' in folder '"<<folder->GetName()<<"' from file '"<<filename<<"'!");
      }
      double error;
...
TGraph* RooLagrangianMorphOptimizer::makeLikelihoodGraph(const int& sample, const TString& parametername, const int n, const double modus){
  ...
  // to-do if(index==-1) ERROR("Did not find parametername "<<parametername<<" in list of used parameters!");
    ...
    while(iss.good()){
      ...
      if (*e != '\0' || errno != 0 ){
        if(defaultvalues.find(pname) != defaultvalues.end()){
         ...
         //to-do ERROR("failed to parse definition of '" << pname << "' and no default value given!");
        }
      }
      pcard[pname] = v;
    }
